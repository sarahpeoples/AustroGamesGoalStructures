...
plotdat <- list()
plotdat[[1]] <- plot_games(Beta=rstan::extract(fit1f,"Beta")$Beta, Q=c(1,1,0,0,0,0,0), M="Land", P="Phylogenetic controls")
plotdat[[2]] <- plot_games(Beta=rstan::extract(fit2f,"Beta")$Beta, Q=c(1,0,1,0,0,0,0), M="Water", P="Phylogenetic controls")
plotdat[[3]] <- plot_games(Beta=rstan::extract(fit4f,"Beta")$Beta, Q=c(1,0,0,0,1,0,0), M="Intra-group", P="Phylogenetic controls")
plotdat[[4]] <- plot_games(Beta=rstan::extract(fit3f,"Beta")$Beta, Q=c(1,0,0,1,0,0,0), M="Intra-cultural", P="Phylogenetic controls")
plotdat[[5]] <- plot_games(Beta=rstan::extract(fit5f,"Beta")$Beta, Q=c(1,0,0,0,0,1,0), M="Inter-cultural", P="Phylogenetic controls")
plotdat[[6]] <- plot_games(Beta=rstan::extract(fit6f,"Beta")$Beta, Q=c(1,0,0,0,0,0,1), M="Stratification", P="Phylogenetic controls")

################################################################################### Standard Models

plotdat[[7]] <- plot_games(Beta=rstan::extract(fit1,"Beta")$Beta, Q=c(1,1,0,0,0,0,0), M="Land", P="Basic")
plotdat[[8]] <- plot_games(Beta=rstan::extract(fit2,"Beta")$Beta, Q=c(1,0,1,0,0,0,0), M="Water", P="Basic")
plotdat[[9]] <- plot_games(Beta=rstan::extract(fit4,"Beta")$Beta, Q=c(1,0,0,0,1,0,0), M="Intra-group", P="Basic")
plotdat[[10]] <- plot_games(Beta=rstan::extract(fit3,"Beta")$Beta, Q=c(1,0,0,1,0,0,0), M="Intra-cultural", P="Basic")
plotdat[[11]] <- plot_games(Beta=rstan::extract(fit5,"Beta")$Beta, Q=c(1,0,0,0,0,1,0), M="Inter-cultural", P="Basic")
plotdat[[12]] <- plot_games(Beta=rstan::extract(fit6,"Beta")$Beta, Q=c(1,0,0,0,0,0,1), M="Stratification", P="Basic")
...
dd$Predictors <- factor(dd$Predictors,levels(dd$Predictors)[c(6, 1:5)])
...
ggsave("Output/FullSet.pdf",pp,height=16,width=16)
...
pp2 <- ggplot(dd2, aes(x=Value, y=1, fill=factor(..quantile..))) + 
	...
	scale_fill_manual(name = "Probability", 
		values = c("#A0A0A0A0", "#D55E00", "#A0A0A0A0"),
    	labels = c("(0, 0.05]", "(0.05, 0.95]", "(0.95, 1]")) + 
    geom_vline(xintercept = 0, linetype="dashed", color = "black", size=0.5) +
  	facet_grid(Model~Predictors, scales = "free") + 
  	labs(x="Change in log relative frequency of cooperative and competitive games", y=" ") +
	theme(text = element_text(size = 16)) + 
	theme(axis.title=element_text(face="bold"),  
		strip.text = element_text(size = 16)) + 
	theme(legend.position = "none") + 
...
ggsave("Output/Difference_coopvscomp.pdf",pp2,height=6,width=16)
...
pp3 <- ggplot(dd3, aes(x=Value, y=1, fill=factor(..quantile..))) + 
  ...
  theme(text = element_text(size = 16)) + theme(axis.title=element_text(face="bold"),  strip.text = element_text(
    size = 16)) + theme(legend.position = "none") + theme(axis.ticks.y = element_blank()) + theme(axis.text.y = element_blank())

ggsave("Output/Difference_cooperative.pdf",pp3,height=6,width=16)
...
ggsave("Output/Difference_competitive.pdf",pp4,height=6,width=16)
...
ggsave("Output/Difference_solitary.pdf",pp5,height=6,width=16)
